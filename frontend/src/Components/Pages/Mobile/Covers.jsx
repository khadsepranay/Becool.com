...
const Covers = () => {
  ...
  const compData=(covComp)=>{
    const sortedData=data.filter((ele)=>{
      return ele.comp==covComp
    })
  ...
  useEffect(()=>{
    const covComp=JSON.parse(localStorage.getItem("cover_company"))
    console.log(covComp)
    compData(covComp)
  },[])
  ...
  return (
    <div className="main_div">
      <h1>Mobile Covers ({sortData.length})</h1>
      <div className="main_cov_div">
        {sortData &&
          sortData.map((ele, ind) => {
            return (
